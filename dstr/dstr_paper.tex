...
One may write \texttt{(vars* \_ (p c1 c2) * (market\_status mission\_state))}
to generate a Cartesian product of variable names such as
\texttt{p\_market\_status} and \texttt{c2\_mission\_state}. Likewise,
\texttt{(domain* *\_mission\_state OPEN RELEASED ...)} applies one finite
domain declaration to every declared variable matching a glob pattern. For
action predicates, \texttt{(assign value to x)} is accepted as readable sugar
for \texttt{(= x+ value)}, \texttt{(equals x y)} as sugar for equality,
...
glob-directed domain declarations, \texttt{assign}, \texttt{equals},
\texttt{unchanged*}, \texttt{if ... then ...}, and
