...
\lstdefinelanguage{DSTR}{
  morekeywords={system,vars,vars*,domain,domain*,init,action,next,invariant,property,
    defmacro,defun,load,quote,set,not,eventually,if,then,assign,to,as,equals,
...
DSTR permits top-level \texttt{defmacro}, \texttt{defun}, and \texttt{load}
forms before the \texttt{system}.  Relative \texttt{load} paths are resolved
against the current \texttt{.dstr} file's directory.
