...
proc bash_needs_wsl_path {exe} {
    # Detect if the bash we're invoking is a WSL bash that requires /mnt/<drive> paths.
    # Cache the result so we only probe once.
    global __bash_needs_wsl_path_cache
    if {[info exists __bash_needs_wsl_path_cache]} {
        return $__bash_needs_wsl_path_cache
    }
    set res 0
    if {![catch {exec $exe -lc {test -d /mnt/c && echo yes || echo no}} out]} {
        if {[string match *yes* $out]} {
            set res 1
        }
    }
    set __bash_needs_wsl_path_cache $res
    return $res
}

proc winpath_to_wsl {p} {
    # Convert c:/path -> /mnt/c/path for WSL bash.
    if {[regexp {^([A-Za-z]):/(.*)} $p -> drive rest]} {
...
proc generaltest {filename {lnum ""}} {
    ...
    fconfigure $out -translation lf -eofchar {}
    puts $out $preamble
    puts $out [read_file_contents $filename]
    close $out
    set rundir [pwd];

    set exec_tmpfile $tmpfile
    if {[string match *bash* $executable] && [bash_needs_wsl_path $executable]} {
        set exec_tmpfile [winpath_to_wsl $tmpfile]
    }

    cd $folder;
    #puts "running from [pwd]";
    if {[catch {exec $executable {*}$args $exec_tmpfile {*}$filterpipe > ${tmpfile}.out} msg]} {
        puts stderr $msg
    }
    cd $rundir;
	...
	exec sed -i {s/\r$//} "${filebase}.golden"    
	exec sed -i {s/\r$//} "${filebase}.out"    
...
proc resttest_helper {filename url endpoint _ _ {lnum ""}} {
	...
	exec sed -i {s/\r$//} "${filebase}.golden"    
	exec sed -i {s/\r$//} "${filebase}.out"    
        catch {exec diff "${filebase}.out"  "${filebase}.golden" > "${filebase}.diff"} msg
        set diffcont [string trim [read_file_contents "${filebase}.diff"]]
