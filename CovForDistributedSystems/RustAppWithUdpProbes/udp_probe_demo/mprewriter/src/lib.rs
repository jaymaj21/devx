//! mprewriter: method-based probe with async UDP, BIG-ENDIAN 20-byte HIT frames
...
fn send_batch(sock: &UdpSocket, addr: &str, batch: &[Msg]) {
    // Pack multiple HIT messages into a single UDP datagram up to ~1440 bytes.
    const MTU_SAFE: usize = 1440;
    let mut buf: Vec<u8> = Vec::with_capacity(MTU_SAFE);
    let mut flush = |b: &mut Vec<u8>| {
        if !b.is_empty() {
    ...
    for m in batch {
        match m {
            Msg::Hit(h) => {
                if buf.len() + 20 > MTU_SAFE { flush(&mut buf); }
                buf.extend_from_slice(&h.message_type.to_be_bytes());
                buf.extend_from_slice(&h.application_id.to_be_bytes());
                buf.extend_from_slice(&h.instance_id.to_be_bytes());
                buf.extend_from_slice(&h.thread_id.to_be_bytes());
                buf.extend_from_slice(&h.stack_depth.to_be_bytes());
                buf.extend_from_slice(&h.loc_id.to_be_bytes());
            }
            Msg::Log(_) => {
                // Logs are variable length; flush hits and send log as its own datagram
                flush(&mut buf);
