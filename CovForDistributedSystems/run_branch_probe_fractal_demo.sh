...
# Build instrumenter, build fractal demo (no source probes), build runtime, instrument demo JAR, run instrumented app.
...
# 2) Fractal demo app
info "Building branch-probe-fractal-demoapp"
...
shopt -u nullglob
[[ -n "$DEMO_IN" ]] || die "Fractal demo input JAR not found"
...
# 3) Runtime
info "Building mprewriter-runtime"
...
# 4) Instrument
info "Instrumenting fractal demo JAR"
EXCL="$ROOT_DIR/branch-probe-fractal-demoapp/exclusions.txt"
INCL="$ROOT_DIR/branch-probe-fractal-demoapp/inclusions.txt"; [[ -f "$INCL" ]] || INCL="$ROOT_DIR/branch-probe-fractal-demoapp/inclusions.example.txt"
EXCL_PROP="$EXCL"; INCL_PROP="$INCL"
if [[ "${OS:-}" == "Windows_NT" ]] && command -v cygpath >/dev/null 2>&1; then
  EXCL_PROP="$(cygpath -w "$EXCL")"; INCL_PROP="$(cygpath -w "$INCL")"
fi
